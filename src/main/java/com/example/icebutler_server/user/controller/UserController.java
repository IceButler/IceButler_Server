...
public class UserController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "400", description = "(U0003)부적절한 소셜로그인 provider 입력입니다.\t\n" +
                    "(G0000)잘못된 파라미터입니다.",
                    content = @Content(schema = @Schema(implementation = ResponseCustom.class))),
            @ApiResponse(responseCode = "403", description = "(U0002)관리자에 의해 서비스 이용이 제한되었습니다.",
                    content = @Content(schema = @Schema(implementation = ResponseCustom.class))),
    })
    @ResponseBody
    @PostMapping("/join")
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "400", description = "(U0003)부적절한 소셜로그인 provider 입력입니다.\t\n" +
                    "(G0000)잘못된 파라미터입니다.",
                    content = @Content(schema = @Schema(implementation = ResponseCustom.class))),
            @ApiResponse(responseCode = "404", description = "(U0001)이미 탈퇴한 회원입니다.",
                    content = @Content(schema = @Schema(implementation = ResponseCustom.class))),
    })
    @ResponseBody
    @PostMapping("/login")
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "400", description = "(U0004)올바르지 않은 닉네임 형식입니다.",
                    content = @Content(schema = @Schema(implementation = ResponseCustom.class)))
    })
    @ResponseBody
